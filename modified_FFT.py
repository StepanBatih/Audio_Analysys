...
# Параметри
wlen = 0.025
wstep = 0.05  # Зменшили крок вікна, щоб більше накладення вікон
FFT_COMPONENTS = 100  # Збільшили кількість компонент FFT
...
for dir in output_dirs:
    for class_name in output_dir_names:
        for path in pathlib.Path(dir + "/" + class_name).glob('**/*.wav'):
            ...
            if fft_features:
                class_data[class_name].append(np.array(fft_features))

# Обробка даних
for key in class_data.keys():
    class_data[key] = [x.reshape(-1, FFT_COMPONENTS) for x in class_data[key]]
    class_data[key] = np.concatenate(class_data[key]) if class_data[key] else np.empty((0, FFT_COMPONENTS))
...
model = Sequential([
    Dense(256, activation='relu', input_shape=(FFT_COMPONENTS,)),
    Dropout(0.3),
...
# Збереження моделі
model.save("fft.h5")
with open("scaler_fft.pkl", 'wb') as file:
    pickle.dump(scaler, file)
with open("label_encoder_fft.pkl", 'wb') as file:
